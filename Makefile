...
backup-db-remote:
	...
	docker run --rm \
		...
		-v $(shell pwd)/backups:/var/www/backups \
		petar1v/docker-baas:1.0 \
		-c "/app/scripts/backup_sql_remote.sh"
...
restore-db-remote:
	...
	docker run --rm \
		...
		-v $(shell pwd)/backups:/var/www/backups \
		petar1v/docker-baas:1.0 \
		-c "/app/scripts/restore_sql_remote.sh"
...
restore-files:
	...
	docker run --rm \
		...
		-v $(shell pwd)/backups:/var/www/backups \
		petar1v/docker-baas:1.0 \
		-c "/app/scripts/restore_files.sh"

# make clean-backups project=site1
clean-backups:
	@if [ -z "$(project)" ]; then \
		echo "Usage: make clean-backups project=site1"; \
		exit 1; \
	fi
	docker run --rm \
		-v $(shell pwd)/backups:/var/www/backups \
